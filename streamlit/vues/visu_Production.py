...
def dashboard_visu_Production():
    ...
    with col1:
        ...
        fig_air = px.line(df, x='Jour', y='Air')
    ...
    with col2:
        st.markdown("<h3 style='color: white; text-align: center; font-family: sans-serif;'>TEMPS D'ASSEMBLAGE MOYEN</h3>", unsafe_allow_html=True)
        
        fig_ass = px.line(df, x='Jour', y='Assemblage')
        fig_ass.update_traces(line_color='#00f2ff', line_width=4, mode='lines+markers')
        fig_ass.update_layout(template="plotly_dark", plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', margin=dict(t=20), font=dict(color="white"))
